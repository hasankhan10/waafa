...
export default function AdminOrdersPage() {
  ...
  const filteredOrders = orders.filter(order => {
    ...
    const clientName = order.full_name || order.profiles?.full_name || "Unknown Customer";
                    ...
                    <React.Fragment key={order.id}>
                      ...
                      >
                        ...
                        <td className="p-5">
                          ...
                          <div className="text-[10px] text-zinc-400 font-sans tracking-wide mt-0.5">{order.email || "No email provided"}</div>
                        </td>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-12 gap-8 animate-in fade-in duration-300">
                                    ...
                                    order.items.map((item, index) => (
                                      <div key={index} className="flex justify-between items-center border-b border-zinc-50 pb-3 last:border-b-0 last:pb-0">
                                        ...
                                        <div className="text-right">
                                          <div className="text-xs font-sans text-zinc-500 font-medium">{item.quantity} x ₹{item.price?.toLocaleString("en-IN")}</div>
                                          <div className="text-xs font-sans text-zinc-900 font-semibold mt-0.5">₹{(item.quantity * item.price)?.toLocaleString("en-IN")}</div>
                                        </div>
                                      </div>
                                    ))
                                  ) : (
                              ...
                              <div className="md:col-span-5 space-y-6">
                                <div className="space-y-4">
                                  ...
                                  <div className="bg-white border border-zinc-100 p-5 text-xs text-zinc-600 leading-relaxed font-sans">
                                    <p className="font-semibold text-zinc-800 mb-1">{clientName}</p>
                                    <p>{order.shipping_address || order.profiles?.address || "No address saved for this order."}</p>
                                  </div>
                                </div>
                                ...
                                <div className="space-y-4">
                                  ...
                                  <div className="bg-white border border-zinc-100 p-5 space-y-3">
                                    <div className="flex items-center gap-3 text-xs text-zinc-600">
                                      <Mail className="h-3.5 w-3.5 text-zinc-400" />
                                      <span className="font-sans">{order.email || "No email on record"}</span>
                                    </div>
                                    <div className="flex items-center gap-3 text-xs text-zinc-600">
                                      <Phone className="h-3.5 w-3.5 text-zinc-400" />
                                      <span className="font-sans">{order.phone || "No phone on record"}</span>
                                    </div>
                                    <div className="flex items-center gap-3 text-xs text-zinc-600">
